...
public class AncientDebrisUtil {
    ...
    public AncientDebrisUtil(PlacedFeature feature, int step, int index) {
        ...
        for (PlacementModifier modifier : feature.placementModifiers()) {
            if (modifier instanceof CountPlacementModifier) this.count = ((CPMAccess) modifier).getCount();
            else if (modifier instanceof HeightRangePlacementModifier) this.heightProvider = ((HRPMAccess) modifier).getHeight();
            else if (modifier instanceof RarityFilterPlacementModifier) this.rarity = ((RFPMAccess) modifier).getChance();
        }
        ...
        } else throw new IllegalStateException("Expected OreFeatureConfig for " + feature);
    }
